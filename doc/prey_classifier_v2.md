...
### Full-data evaluation (all 655 labelled bursts)

End-to-end run over every burst with `human_prey ∈ {0,1}` using
door-gate semantics (`door opens ⟺ cat detected AND prey_pred=0 AND
direction != exiting`). Generated by
[tools/eval_full_pipeline.py](../tools/eval_full_pipeline.py);
results in [`models/prey_v3/full_eval/summary.txt`](../models/prey_v3/full_eval/summary.txt).

| pipeline | TP open | TN block | FP block | **FN safety** | open-correct | block-correct |
|---|---:|---:|---:|---:|---:|---:|
| yolo11x + bodyA       | 261 | 27 | 19 | **1** | 93.2 % | 96.4 % |
| **yolo11s + bodyA_s** | 254 | 25 |  0 | **1** | **100.0 %** | 96.2 % |

By time-of-day (all false-blocks are night-only):

| | x night FP | s night FP | x day FP | s day FP |
|---|---:|---:|---:|---:|
| FP block | 19 | **0** | 0 | 0 |

By cat:
- mazge (147 bursts): x 16 FP → s 0 FP, same single FN
- benis (74 bursts): x 1 FP → s 0 FP, no FN either pipeline

Both pipelines miss the same prey burst `20260505_220948_gen9` —
a blown-out close-up where the classifier fails on either set of
