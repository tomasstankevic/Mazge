...
def serve(remoteAddr, localAddr, remotePort, localPort, password, filename, command = FLASH):
  ...
  try:
    ...
    # Send data with small delays to let ESP32 write to flash
    import time
    while True:
      chunk = f.read(1024)
      if not chunk: break
      ...
      try:
        ...
        sys.stderr.write('\n')
        logging.error('Error Uploading: %s', ex)
        connection.close()
        f.close()
        sock.close()
        return 1
    ...
    sys.stderr.write('\n')
    logging.info('All data sent (%d bytes), waiting for result...', offset)
    try:
      count = 0
      while True:
        count=count+1
      ...
      logging.error('Final result: %s', e)
      connection.close()
      f.close()
      sock.close()
      return 1
